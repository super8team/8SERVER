...
AppController = function() {
  ...
  this.packageExample = document.getElementById('packageDiv');
...
AppController.prototype.onTab = function() {
  ...
  if (this.selectedTab ==  AppController.BLOCK_FACTORY) {
    ...
    var blockTypes = this.blockLibraryController.getStoredBlockTypes();0
...
AppController.prototype.assignLibraryClickHandlers = function() {
  ...
  //패키지 생성하기
  document.getElementById('createNewPackage').addEventListener('click',
      function(){
  ...
  //패키지 하나를 클릭 했으 때
  // document.getElementById('packageDiv').addEventListener('click',
  //     $('#packageDiv').click(function(e){
  //       //콘텐츠 packagenum 값 가져오기
  //       var getBlockCode = FactoryUtils.getBlockCode();
  //           getBlockCode = JSON.parse(getBlockCode);
  //
  //       //packagenum값을 변수에 담음
  //       var primary_packagenum = getBlockCode.packagenum;
  //
  //       //패키지의 고유 아이디 번호
  //       var package_id     = e.target.getAttribute('id');
  //
  //       //콘텐츠가 가지고 있는 아이디값이랑 패키지가 가지고 있는 아이디 값이 옳은지 확인 해 봅시다
  //       var button_blockLib = document.getElementById('dropdownDiv_blockLib');
  //       //콘텐츠들
  //       var input_tag       = button_blockLib.getElementsByTagName('input');
  //
  //
  //
  ...
  //       {
  //         //콘텐츠가 가지고 있는 아이디 값
  //         var ex = input_tag[i].id.split('_');
  //         if(package_id == ex[1])
  //         {
  //           alert('disabled'+input_tag[i].disabled);
  //           input_tag[i].disabled = 'true';
  //           alert(input_tag[i].disabled);
  //         }
  //         //ex[0] = dropdown_
  //         //ex[1] = {primary key}
  //         //콘텐츠 아이디 프라이머리 키
  //         console.log(ex[1]);
  //       };
  //       console.log("배열의 길이:"+input_tag.length);
  //       console.log("아이디값:"+id);
  //       return;
  //     }));
};
