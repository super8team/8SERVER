...
FactoryUtils.formatJson_ = function(blockType, rootBlock) {
  ...
  console.log(contentsBlock);
  // console.log(contentsBlock.getInputTargetBlock);
  // console.log(contentsBlock.getInputTargetBlock('FIELDS'));
  var lastInput = null;
  var ifNameNumber = 1;
  while (contentsBlock) {
    if (!contentsBlock.disabled && !contentsBlock.getInheritedDisabled()) {
      ...
      var conrtrols = FactoryUtils.getFieldsJson_(
          contentsBlock.getInputTargetBlock('CONTROLS'));
      ...
      console.log("루트블럭"+rootBlock);
      // alert("^^");
      // alert(rootBlock.match('red');
    }
  ...
  console.log("message:"+JS.message0);
  if (args.length) {
    JS.args0 = args;
  }
  ...
  var number = rootBlock.getInputTargetBlock('BUTTON1');
  // JS.boolean =
  ...
  JS.context = FactoryUtils.getConTextFromRootBlock_(rootBlock);
  JS.button1 = FactoryUtils.getButtonBoolRootBlock_(rootBlock);
  JS.button2 = FactoryUtils.getButtonBoolRootBlock1_(rootBlock);
  // JS.variable = FactoryUtils.getButtonBoolRootBlock2_(rootBlock);
 ...
 * @private
 */
 FactoryUtils.getControlsJson_ = function(block) {
   var controls = [];
   ...
   while (block) {
     if (!block.disabled && !block.getInheritedDisabled()) {
       switch (block.type) {
         case 'logic_compare':
           // Result: 'hello'
           controls.push(block.getFieldValue('TEXT'));
           break;
...
FactoryUtils.getButtonBoolRootBlock_ = function(rootBlock) {
  var contextBlock = rootBlock.getInputTargetBlock('BUTTON1');
  console.log(contextBlock);
  if (contextBlock && !contextBlock.disabled) {
    return contextBlock.getFieldValue('BOOL');
  }
  return '';
};

FactoryUtils.getButtonBoolRootBlock1_ = function(rootBlock) {
  var contextBlock = rootBlock.getInputTargetBlock('BUTTON2');
...
FactoryUtils.getButtonBoolRootBlock2_ = function(rootBlock) {
  var context = rootBlock.getInputTargetBlock('VARI');
  console.log(context);
  if(context && !contextBlock.disabled){
